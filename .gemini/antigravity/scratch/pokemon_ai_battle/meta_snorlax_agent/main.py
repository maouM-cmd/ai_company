"""Meta deck: カビゴン Stall - たおれこむ160+ねむり / おおぐいでエネ加速"""
...
SNORLAX = 1072
LUNATONE = 675; SOLROCK = 676
BASIC_FIGHTING = 6
ULTRA_BALL = 1121; SWITCH = 1123; BOSS = 1182; LILLIE = 1227
CARMINE = 1192; JUDGE = 1213; STRETCHER = 1097; HAMMER = 1120
POKE_PAD = 1152; CAPE = 1159; ENERGY_PRO = 1100

def read_deck_csv():
    return ([SNORLAX]*4 + [LUNATONE]*3 + [SOLROCK]*3 +
            [BASIC_FIGHTING]*16 + [ULTRA_BALL]*4 + [SWITCH]*4 +
            [BOSS]*4 + [LILLIE]*4 + [CARMINE]*4 + [JUDGE]*2 +
            [STRETCHER]*3 + [HAMMER]*4 + [POKE_PAD]*2 +
            [JUDGE]*2 + [ENERGY_PRO]*1)
...
def get_energies(poke):
    try: return len(getattr(poke, 'energies', []) or [])
    except: return 0

def select_main_action(obs):
    options = obs.select.option
    attacks, attaches, plays, retreats = [], [], [], []
    end_idx = None
    ...
    if plays:
        ...
        for idx in plays:
            ...
            if cid in (SNORLAX, LUNATONE, SOLROCK):
                if bench_count < 5: allowed.append(idx)
            else: allowed.append(idx)
        prio = [ULTRA_BALL, CAPE, POKE_PAD, BOSS, LILLIE, CARMINE, JUDGE, SWITCH, STRETCHER, HAMMER, ENERGY_PRO]
        for cid in prio:
            for idx in allowed:
    ...
    if attacks:
        # おおぐい(attack[0]) でエネ加速 → 4エネ貯まったらたおれこむ(attack[1])
        if active_id == SNORLAX:
            total_e = get_energies(active)
            if total_e >= 4 and len(attacks) > 1: return [attacks[1]]  # たおれこむ160
            return [attacks[0]]  # おおぐいでエネ加速
        return [attacks[-1]]
...
def select_card(obs):
    ...
    if context == SelectContext.SETUP_ACTIVE_POKEMON:
        for i, opt in enumerate(options):
            if get_option_card_id(opt, obs) == SNORLAX: return [i]
    ...
    if context in (SelectContext.SETUP_BENCH_POKEMON, SelectContext.TO_BENCH):
        ...
        for i, opt in enumerate(options):
            ...
            score = 100 if cid == SNORLAX else (60 if cid in (LUNATONE, SOLROCK) else 0)
    ...
    if context == SelectContext.DISCARD:
        scored = [(0 if get_option_card_id(opt, obs) in (SNORLAX, LUNATONE, SOLROCK) else
                   40 if get_option_card_id(opt, obs) == BASIC_FIGHTING else 70, i) for i, opt in enumerate(options)]
        scored.sort(key=lambda x: x[0], reverse=True)
        count = max(min_count, min(max_count, len(scored)))
        return [x[1] for x in scored[:count]]
    ...
    if is_search:
        scored = [(100 if get_option_card_id(opt, obs) == SNORLAX else
                   60 if get_option_card_id(opt, obs) in (LUNATONE, SOLROCK) else
                   40 if get_option_card_id(opt, obs) == BASIC_FIGHTING else 20, i) for i, opt in enumerate(options)]
        scored.sort(key=lambda x: x[0], reverse=True)
