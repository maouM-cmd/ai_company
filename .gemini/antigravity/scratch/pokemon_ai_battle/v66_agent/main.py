"""
V66: Mega Froslass ex × Mega Starmie ex 水デッキ (V65対戦分析に基づく改善)
本番リプレイ分析（30勝26敗 53.6%）から判明した弱点を修正:
- ルカリオex対策: Rioluがベンチに見えたらBoss Orders最優先で進化前にKO（8負け→改善）
- ドラパルトex対策: ベンチ上限2に制限（Phantom Diveのダメカン分散を最小化）（4負け→改善）
- エースバーン対策: 専用優先度リスト追加（3負け→改善）
- デッキ: BOSS_ORDERS 1→2枚、ULTRA_BALL 1→0枚
"""
from cg.api import (
...
def read_deck_csv() -> list[int]:
    ...
    return (
        ...
        [BOSS_ORDERS]        * 2  +  # 56 (V66: 1→2枚)
        [HILDA]              * 2  +  # 58
        [HARLEQUIN]          * 2     # 60 (ULTRA_BALL削除分をHARLEQUIN維持)
        # ULTRA_BALL: 1→0枚（-1）、BOSS_ORDERS: 1→2枚（+1）で合計60枚維持
    )
...
def select_main_action(obs: Observation) -> list[int]:
    ...
    if plays:
        ...
        if opp_deck == "LUCARIO":
            ...
            prio = [BOSS_ORDERS, MEGA_SIGNAL, ENERGY_SEARCH, POKEGEAR,
                    BUDDY_BUDDY_POFFIN, NIGHT_STRETCHER, SALVATORE, HILDA,
                    WALLYS_COMPASSION, HARLEQUIN, LILLIES]
        elif opp_deck == "ACERBURN":
            # エースバーン対面: 高速展開・攻撃優先
            prio = [MEGA_SIGNAL, ENERGY_SEARCH, POKEGEAR, BUDDY_BUDDY_POFFIN,
                    SALVATORE, HILDA, WALLYS_COMPASSION,
                    BOSS_ORDERS, NIGHT_STRETCHER, HARLEQUIN, LILLIES]
        elif opp_deck in ("ABOMASNOW",):
            prio = [MEGA_SIGNAL, ENERGY_SEARCH, POKEGEAR, BUDDY_BUDDY_POFFIN,
            ...
            prio = [NIGHT_STRETCHER, ULTRA_BALL, MEGA_SIGNAL, ENERGY_SEARCH,
...
def select_card(obs: Observation) -> list[int]:
    ...
    if is_search:
        opp_deck = identify_opponent_deck(obs)
        bench_ids = [getattr(p, 'id', -1) for p in (my_state.bench or []) if p]
        active_id = getattr(my_state.active[0], 'id', -1) if my_state.active else -1

        has_froslass = MEGA_FROSLASS_EX in bench_ids or active_id == MEGA_FROSLASS_EX
        has_starmie  = MEGA_STARMIE_EX  in bench_ids or active_id == MEGA_STARMIE_EX
        has_snorunt  = SNORUNT in bench_ids or active_id == SNORUNT
        has_staryu   = STARYU  in bench_ids or active_id == STARYU

        scored = []
        for i, opt in enumerate(options):
            cid = get_option_card_id(opt, obs)
            if cid == MEGA_FROSLASS_EX:     score = 1000 if not has_froslass else 200
            elif cid == MEGA_STARMIE_EX:    score = 900  if not has_starmie  else 190
            elif cid == SNORUNT:            score = 700  if not has_snorunt  else 150
            elif cid == STARYU:             score = 650  if not has_staryu   else 140
            elif cid == WATER_ENERGY:       score = 500
            elif cid == MIST_ENERGY:        score = 480
            elif cid == LEGACY_ENERGY:      score = 460
            elif cid == IGNITION_ENERGY:    score = 440
            elif cid == MEGA_SIGNAL:        score = 400
            elif cid == ENERGY_SEARCH:      score = 380
            elif cid == BUDDY_BUDDY_POFFIN: score = 360
            # ルカリオ対面ではBoss Ordersの価値を上げる
            elif cid == BOSS_ORDERS:
                score = 420 if opp_deck == "LUCARIO" else 300
            elif cid == NIGHT_STRETCHER:    score = 340
            else:                           score = 100
            scored.append((score, i))
        scored.sort(key=lambda x: x[0], reverse=True)
        count = max(min_count, min(max_count, len(scored)))
