"""V67 vs V69 A/B 比較 — 同じ seeds で両エージェントを測定して公平比較"""
...
V69 = load("v69", os.path.join(BASE, "v69_agent/main.py"))
...
N = 20  # V67+V69 同じ seeds で測定（高速版）

print("=" * 65)
print(f"=== V67 vs V69 A/B 比較 (N={N} each) ===")
print("=" * 65)
print(f"  {'対面':12s}  V67   V69   差分  判定")
print("-" * 65)

total_v67_w = total_V69_w = total_g = 0

for meta_name, meta_mod in meta_agents:
    v67_wins = V69_wins = errors = 0

    for seed in range(N):
        r67 = run_one(v67, meta_mod, seed)
        r68 = run_one(V69, meta_mod, seed)
    ...
    wr68 = V69_wins / valid * 100 if valid > 0 else 0
    diff = wr68 - wr67
    mark = "↑" if diff > 2 else ("↓" if diff < -2 else "→")

    total_v67_w += v67_wins
    total_V69_w += V69_wins
    total_g += valid
...
overall_V69 = total_V69_w / total_g * 100 if total_g > 0 else 0
overall_diff = overall_V69 - overall_v67
verdict = "V69が上" if overall_diff > 1 else ("V67が上" if overall_diff < -1 else "ほぼ同等")

print(f"  {'総合':12s}  {overall_v67:5.1f}%  {overall_V69:5.1f}%  {overall_diff:+5.1f}%  {verdict}")
print()
print(f"V67: {total_v67_w}/{total_g}, V69: {total_V69_w}/{total_g}")
