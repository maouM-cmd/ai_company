"""Meta deck: メガユキノオーex - アバランチハンマー100×水エネ数 / フロストバリア200"""
...
YUKIKABURI = 722; MEGA_ABOMASNOW_EX = 723
BASIC_WATER = 3
ULTRA_BALL = 1121; SWITCH = 1123; BOSS = 1182; LILLIE = 1227
CARMINE = 1192; JUDGE = 1213; STRETCHER = 1097; MEGA_SIGNAL = 1145
MITSURU = 1229; CAPE = 1159; WAITRESS = 1235; HAMMER = 1120

def read_deck_csv():
    return ([YUKIKABURI]*4 + [MEGA_ABOMASNOW_EX]*4 + [BASIC_WATER]*15 +
            [MEGA_SIGNAL]*4 + [SWITCH]*4 + [BOSS]*4 + [LILLIE]*4 +
            [CARMINE]*4 + [ULTRA_BALL]*4 + [JUDGE]*2 + [STRETCHER]*2 +
            [MITSURU]*1 + [JUDGE]*2 + [WAITRESS]*4 + [HAMMER]*2)
...
def select_main_action(obs):
    ...
    if evolves:
        for idx in evolves:
            if get_option_card_id(options[idx], obs) == MEGA_ABOMASNOW_EX: return [idx]
        return [evolves[0]]
    ...
    if plays:
        ...
        prio = [MEGA_SIGNAL, WAITRESS, ULTRA_BALL, CAPE, BOSS, LILLIE, CARMINE, JUDGE, SWITCH, STRETCHER, HAMMER]
        for cid in prio:
            for idx in allowed:
...
def select_card(obs):
    ...
    if context == SelectContext.SETUP_ACTIVE_POKEMON:
        for i, opt in enumerate(options):
            if get_option_card_id(opt, obs) == YUKIKABURI: return [i]
    ...
    if context in (SelectContext.SETUP_BENCH_POKEMON, SelectContext.TO_BENCH):
        ...
        for i, opt in enumerate(options):
            ...
            score = 100 if cid == YUKIKABURI else 50
    ...
    is_opp_bench = len(options) > 0 and all(
        getattr(opt, 'area', None) == AreaType.BENCH and getattr(opt, 'playerIndex', None) == opp_idx
        for opt in options)
    if is_opp_bench:
        scored = []
        for i, opt in enumerate(options):
    ...
    if context == SelectContext.DISCARD:
        ...
        for i, opt in enumerate(options):
            cid = get_option_card_id(opt, obs)
            score = 0 if cid in (MEGA_ABOMASNOW_EX, YUKIKABURI) else (40 if cid == BASIC_WATER else 70)
            scored.append((score, i))
        scored.sort(key=lambda x: x[0], reverse=True)
        count = max(min_count, min(max_count, len(scored)))
        return [x[1] for x in scored[:count]]
    ...
    if is_search:
        ...
        for i, opt in enumerate(options):
            cid = get_option_card_id(opt, obs)
            score = 100 if cid == MEGA_ABOMASNOW_EX else (80 if cid == YUKIKABURI else (50 if cid == BASIC_WATER else 30))
            scored.append((score, i))
        scored.sort(key=lambda x: x[0], reverse=True)
