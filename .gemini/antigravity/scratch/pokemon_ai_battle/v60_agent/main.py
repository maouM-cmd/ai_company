"""
V60: エースバーン × Mega Starmie ex 水デッキ
- 上位1-2位 (keidroid / Mogja J) と完全同一構成
- Cinderace (HP160, Stage2): Ignition Energyで高火力
- Mega Starmie ex (HP330, Stage1): 超耐久サブアタッカー
- Crushing Hammer で相手エネルギー破壊
"""
from cg.api import (
...
def select_main_action(obs: Observation) -> list[int]:
    ...
    if plays:
        ...
        if bench_count < max_bench:
            has_staryu = any(getattr(p, 'id', -1) in (STARYU, MEGA_STARMIE_EX)
                             for p in (my_state.bench or []) if p)
            if not has_staryu:
                for idx in allowed:
    ...
    # 4. エネルギー付け
    if attaches:
        # Cinderace(エネ0)のとき: Ignition Energy優先 (1枚で●●●=Turbo Flare即撃ち可能)
        if active_id == CINDERACE and active_energy == 0:
            for idx in attaches:
    ...
    if context in (SelectContext.ATTACH_TO, SelectContext.ATTACH_FROM):
        ...
        if is_choosing_pokemon:
            ...
            for i, opt in enumerate(options):
                ...
                e = get_energies(poke)
                # ユキノオー/ルカリオ対面: Turbo Flareでもベンチ Starmie ex優先
                if opp_deck_at in ("ABOMASNOW", "LUCARIO"):
                    if cid == MEGA_STARMIE_EX: score = 900 - e * 50
                    elif cid == STARYU:        score = 700
                    elif cid == CINDERACE:     score = 400 - e * 50
                    else:                      score = 0
                else:
